...
def configure_chinese_fonts() -> None:
    preferred = ["Hiragino Sans GB", "STHeiti", "Songti SC", "Songti", "Arial Unicode MS"]
    available = []
    for name in preferred:
        try:
            ...
            continue
    if available:
        plt.rcParams["font.sans-serif"] = available + plt.rcParams.get("font.sans-serif", [])
...
def save_prediction_plot(time: pd.Series | np.ndarray, predicted: np.ndarray, path: Path, title: str, observed: np.ndarray | None = None) -> None:
    ...
    if observed is not None:
        ax.plot(time, observed, lw=1.0, label="观测值", color="#1f77b4")
    ax.plot(time, predicted, lw=1.3, label="预测值", color="#d62728")
    ax.set_title(title)
